...
type RestOf<T extends IMessage | NewMessage> = Omit<T, 'receivedAt'> & {
...
export class MessageService {
  ...
  protected convertDateFromClient<T extends IMessage | NewMessage | PartialUpdateMessage>(message: T): RestOf<T> {
    return {
      ...message,
      receivedAt: message.receivedAt?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restMessage: RestMessage): IMessage {
    return {
      ...restMessage,
      receivedAt: restMessage.receivedAt ? dayjs(restMessage.receivedAt) : undefined,
    };
  }
